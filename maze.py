...
class MazeArray():
    ...
    def valid_move(self, nextpos: Cell):
        ...
        if self.agent[0] > nextpos.x: return not self.cell_at(self.agent).walls['W']
        if self.agent[0] < nextpos.x: return not self.cell_at(self.agent).walls['E']
        if self.agent[1] > nextpos.y: return not self.cell_at(self.agent).walls['N']
        if self.agent[1] < nextpos.y: return not self.cell_at(self.agent).walls['S']
